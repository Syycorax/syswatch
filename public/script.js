...
function getTemperatureData(id) {
    fetch(BASEURL + "/temp", {
        ...
        for (var i = 0; i < data.length; i++) {
            if (data[i].id == id) {
                console.log("Temperature data: ", data[i].temp);
                document.querySelector(".temp").textContent = "Temperature: " + data[i].temp;
                updateprogresssmooth(document.querySelector(".temp-progress").value, data[i].temp, ".temp-progress");
            }
        }
    }
    );
